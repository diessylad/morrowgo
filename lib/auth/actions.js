...
export async function resendConfirmationAction(previous, form) {
  ...
  if (!validEmail(email)) return { error: 'Enter a valid email address.' };
  try {
    await client.auth.resend({ type: 'signup', email, options: { emailRedirectTo: `${siteOrigin()}/auth/callback` } });
  } catch { /* Uniform result prevents account enumeration, including already-confirmed accounts. */ }
  return { success: 'If your email is waiting for confirmation, a new link will arrive shortly. Check spam too. Wait a minute before trying again. Already confirmed? Sign in instead.' };
}

export async function oauthAction(previous, form) {
  const provider = field(form, 'provider');
  if (!['apple', 'google'].includes(provider)) return { error: 'Choose Apple or Google to continue.' };
  const client = createServerSupabaseClient();
  if (!client) return unavailable;
  if (!await isOAuthProviderEnabled(provider)) return { error: 'This sign-in option is temporarily unavailable. Please use email or try again later.' };
  let destination;
  try {
    const { data, error } = await client.auth.signInWithOAuth({
      provider,
      options: { redirectTo: `${siteOrigin()}/auth/callback`, skipBrowserRedirect: true }
    });
    if (error || !data?.url) return { error: 'This sign-in option is temporarily unavailable. Please use email or try again later.' };
    destination = data.url;
  } catch { return { error: 'This sign-in option is temporarily unavailable. Please use email or try again later.' }; }
