...
export function normalizeCatalogue(rows) {
  ...
  for(const row of rows) {
    ...
    for(const op of row.operators||[]) for(const p of op.packages||[]) {
      ...
      packages.push({id:p.id,country:row.title,iso:row.country_code,price,currency:'EUR',dataMB:Number(p.amount)||0,dataGB:(Number(p.amount)||0)/1024,duration:Number(p.day),unlimited:!!p.is_unlimited,operator:op.title,networks,topupAvailable:!!op.rechargeability,packageType:op.plan_type||'data',fairUsage:p.is_fair_usage_policy?String(p.fair_usage_policy||'Fair-use policy applies'):null,title:p.title,sandbox:true});
    }
