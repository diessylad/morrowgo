...
export default function OrderList({ orders, demo = false }) {
  return <div className={styles.orderList}>{orders.map(order => {
    ...
    return <article className={styles.orderRow} key={order.id}><div className={styles.orderMain}><h3>{order.planName || 'Travel eSIM order'}</h3><p>MORROWGO order {order.id}</p><p>{date}{demo ? ' · sample order' : ''}</p></div><div className={styles.orderMeta}>{price && <strong>{price}</strong>}<span className={styles.badge}>{labels[order.status] || 'Status unavailable'}</span></div></article>;
  })}</div>;
}
