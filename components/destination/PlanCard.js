...
export default function PlanCard({plan,recommended,formatData,onBuy}) {
 ...
 return <article className={`${s.card} ${recommended?s.recommended:''}`}>
  ...
  <div className={s.buy}><button onClick={()=>onBuy(plan)} aria-label={`Buy ${formatData(plan)}, ${plan.duration} days, ${price}`}>Buy now <ArrowRight size={17}/></button><span>{speed || (plan.packageType==='data'?'Data only':'Data + calls + texts')}</span></div>
 </article>;
}
