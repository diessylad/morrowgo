...
export default function PlanCard({plan,recommended,formatData,onBuy,showSpeedDetails=false}) {
 ...
 return <article className={`${s.card} ${recommended?s.recommended:''}`}>
  ...
  <div className={s.buy}><button onClick={()=>onBuy(plan)} aria-label={`Buy ${formatData(plan)}, ${plan.duration} days, ${price}`}>Buy now <ArrowRight size={17}/></button><span>{[speed, plan.packageType==='data'?'Data only':plan.packageType?'Data + calls + texts':''].filter(Boolean).join(' · ')}</span></div>
